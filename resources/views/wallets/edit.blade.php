                    ...
                    <div>
                        ...
                        <select id="type"
                            ...
                            @foreach($types as $key => $label)
                                <option value="{{ $key }}" {{ old('type', $wallet->type) == $key ? 'selected' : '' }}>
                                    {{ $label }}
                                </option>
                            @endforeach
                        </select>
                            ...
                            @foreach($defaultColors as $color)
                                <label class="cursor-pointer">
                                    <input type="radio"
                                           ...
                                           x-model="form.color"
                                           class="sr-only"
                                        {{ old('color', $wallet->color) == $color ? 'checked' : '' }}>
    ...
    @push('scripts')
        <script>
            function walletForm() {
                return {
                    form: {
                        name: '{{ old("name", $wallet->name) }}' || '',
                        type: '{{ old("type", $wallet->type) }}' || '',
                        balance: '{{ old("balance", $wallet->balance) }}' || '0.00',
                        color: '{{ old("color", $wallet->color) }}' || '#0b4c64',
