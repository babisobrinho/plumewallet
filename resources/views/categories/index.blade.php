...
<div class="container mx-auto px-4 py-8">
    ...
    <div class="relative mb-16">
        ...
        <div class="bg-white p-10 rounded-xl shadow-lg border border-plume-blue-100">
            <div class="flex flex-col items-center justify-center py-6">
                <h3 class="text-2xl font-semibold text-plume-blue-600 mb-6">Saldo Disponível</h3>
                <span class="text-5xl font-extrabold @if($balance >= 0) text-plume-teal-600 @else text-plume-red-600 @endif">
            ...
            <div class="space-y-8">
                @foreach($groupedCategories as $date => $categoryItems)
                    <div class="space-y-4">
                        ...
                        @endphp
                        <p class="font-semibold text-gray-500">{{ $dateObj->translatedFormat('d F, Y') }}</p>
                            ...
                            @foreach($categoryItems as $item)
                                ...
                                <div class="bg-white rounded-xl shadow-lg p-6 border-l-4 border-plume-{{ $category->color }}">
                                    <div class="flex items-center mb-4">
                                        ...
                                        <div class="flex flex-col items-end">
                                            @if(!$item['is_creation'] && $item['transaction_amount'])
                                                <span class="text-lg font-bold @if($item['transaction_amount'] < 0) text-plume-red-500 @else text-plume-teal-500 @endif">
                                            ...
                                            @else
                                                <span class="text-lg font-bold @if($category->transactions_sum_amount < 0) text-plume-red-500 @else text-plume-teal-500 @endif">
                                    ...
                                    <div class="flex justify-end mt-4 space-x-4 border-t pt-4">
                                        <a href="{{ route('categories.transactions', $category) }}" class="text-plume-blue-500 hover:text-plume-blue-700 transition duration-300 ease-in-out text-xl">
                                        ...
                                        @if($category->transactions_count > 0)
                                            @if(isset($item['transaction']))
                                                <a href="{{ route('transactions.edit', $item['transaction']) }}" class="text-plume-blue-500 hover:text-plume-blue-700 transition duration-300 ease-in-out text-xl">
                                                    <i class="fas fa-edit"></i>
                                                </a>
                                            @else
                                                <a href="{{ route('transactions.edit', $category->transactions()->latest()->first()) }}" class="text-plume-blue-500 hover:text-plume-blue-700 transition duration-300 ease-in-out text-xl">
                                                    <i class="fas fa-edit"></i>
                                                </a>
                                            @endif
                                        @else
                                            <a href="{{ route('categories.edit', $category) }}" class="text-plume-blue-500 hover:text-plume-blue-700 transition duration-300 ease-in-out text-xl">
                                                <i class="fas fa-edit"></i>
                                            </a>
                                        @endif
                                        ...
                                        <form action="{{ route('categories.destroy', $category) }}" method="POST" onsubmit="return confirm('Tem certeza que deseja apagar esta categoria? Todas as transações associadas também serão removidas.')">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="text-plume-red-500 hover:text-plume-red-700 transition duration-300 ease-in-out text-xl">
